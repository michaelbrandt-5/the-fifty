...
- The old machine's Node was a **manual tarball install at `~/local/nodejs/node-v22.15.0-darwin-arm64/`** (no Homebrew/nvm at the time), later joined by a system Node 24 at `/usr/local/bin/node`. `.claude/launch.json` still pointed at the tarball path — **fixed 2026-07-05**: it now runs plain `npm run dev` (new machine's Node is Homebrew v26.4.0 at `/opt/homebrew/bin`).
- **Puppeteer's Chrome download silently corrupts under Node 26** (verified 2026-07-05). `@puppeteer/browsers` 2.13.0 (bundled with puppeteer 24.x) extracts zips via the unmaintained `extract-zip`, whose promise never settles on large archives under Node v26.4.0 — the process **exits 0** mid-extraction, leaving a ~420 KB hollow app bundle plus the un-deleted `.zip` in `~/.cache/puppeteer`, and prerender later fails with dlopen "Google Chrome for Testing Framework … no such file". Repair: `cd ~/.cache/puppeteer/chrome && rm -rf mac_arm-<build> && mkdir mac_arm-<build> && ditto -x -k <build>-chrome-mac-arm64.zip mac_arm-<build>/` (same for `chrome-headless-shell`), then delete the zips. The cache is currently intact, so installs/postinstalls no-op; the trap re-arms whenever puppeteer's pinned browser build changes. Real fix: upgrade puppeteer/puppeteer-core to 25.x (`@puppeteer/browsers` 3.x replaced extract-zip with modern-tar) — remember the Vercel lockstep bump of `@sparticuz/chromium-min` + `CHROMIUM_PACK_URL` in `scripts/prerender.mjs`.
